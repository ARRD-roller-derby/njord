...
export default async function generalRanking(req: NextApiRequest, res: NextApiResponse) {
  ...
  const ranking = await User.find(where).skip(page > 1 ? page * perPage - perPage : 0).select('avatar lastname name derbyName numRoster email dailyContestAvgAccuracy dailyContestAvgTime').limit(perPage).sort({ [type]: direction })
